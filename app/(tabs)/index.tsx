...
// Tipagem para os itens de senha
type PasswordItem = {
  name: string;
  password: string;
};
...
export default function Home() {
  ...
  const savePassword = async () => {
    ...
    const newEntry: PasswordItem = { name, password };
  ...
  const loadPasswords = async () => {
    try {
      ...
      if (data) {
        const parsedData: PasswordItem[] = JSON.parse(data);
  ...
  const deletePassword = async (indexToDelete: number) => {
    const updatedList = savedPasswords.filter((_, index) => index !== indexToDelete);
    setSavedPasswords(updatedList);
    await SecureStore.setItemAsync('passwords', JSON.stringify(updatedList));
  };
